...
public class DruidDataSource extends DruidAbstractDataSource implements DruidDataSourceMBean, ManagedDataSource, Referenceable, Closeable, Cloneable, ConnectionPoolDataSource, MBeanRegistration {
  ...
  boolean killWhenSocketReadTimeout=false;
  static AtomicLongFieldUpdater<DruidDataSource> recycleErrorCountUpdater=AtomicLongFieldUpdater.newUpdater(DruidDataSource.class,"recycleErrorCount");
  static AtomicLongFieldUpdater<DruidDataSource> connectErrorCountUpdater=AtomicLongFieldUpdater.newUpdater(DruidDataSource.class,"connectErrorCount");
  static AtomicLongFieldUpdater<DruidDataSource> resetCountUpdater=AtomicLongFieldUpdater.newUpdater(DruidDataSource.class,"resetCount");
    ...
    }
{
  ...
  public void init() throws SQLException {
    ...
    try {
      ...
      if (JdbcConstants.MYSQL.equals(this.dbType) || JdbcConstants.MARIADB.equals(this.dbType) || JdbcConstants.ALIYUN_ADS.equals(this.dbType)) {
 ...
 */
 ...
 else     if (realDriverClassName.equals(JdbcConstants.POSTGRESQL_DRIVER) || realDriverClassName.equals(JdbcConstants.ENTERPRISEDB_DRIVER)) {
  ...
  private void initExceptionSorter(){
    ...
    for (Class<?> driverClass=driver.getClass(); ; ) {
      ...
      if (realDriverClassName.equals(JdbcConstants.MYSQL_DRIVER) || realDriverClassName.equals(JdbcConstants.MYSQL_DRIVER_6)) {
    ...
    for (boolean createDirect=false; ; ) {
      ...
      try {
        ...
        if (onFatalError && onFatalErrorMaxActive > 0 && activeCount >= onFatalErrorMaxActive) {
          ...
          String errorMsg="onFatalError, activeCount " + activeCount + ", onFatalErrorMaxActive "+ onFatalErrorMaxActive;
 ...
 */
  ...
  @Override public int getPoolingCount(){
    lock.lock();
    try {
      return poolingCount;
    }
    ...
    private void runInternal(){
      for (; ; ) {
        ...
        try {
          ...
          if (emptyWait) {
            if (poolingCount >= notEmptyWaitThreadCount && !(keepAlive && activeCount + poolingCount < minIdle) && !initTask) {
              createTaskCount--;
              return;
            }
    ...
    public void run(){
      ...
      for (; ; ) {
        ...
        try {
          ...
          LOG.error("create connection SQLException, url: " + jdbcUrl + ", errorCode "+ e.getErrorCode()+ ", state "+ e.getSQLState(),e);
          errorCount++;
          if (errorCount > connectionErrorRetryAttempts && timeBetweenConnectErrorMillis > 0) {
            setFailContinuous(true);
            if (failFast) {
 ...
 */
  ...
  @Override public JdbcDataSourceStat getDataSourceStat(){
  ...
  @SuppressWarnings("unchecked") public <T>T unwrap(  Class<T> iface){
    for (    Filter filter : this.filters) {
      if (filter.isWrapperFor(iface)) {
        return (T)filter;
      }
  ...
  @Override public int fill() throws SQLException {
  ...
  @Override public ObjectName preRegister(  MBeanServer server,  ObjectName name) throws Exception {
    if (server != null) {
      try {
  ...
  @Override public void postDeregister(){
  }

...
public class MySqlStatementParser extends SQLStatementParser {
  ...
  public SQLStatement parseCreate(){
    ...
    if (lexer.token() == Token.UNIQUE || lexer.token() == Token.INDEX || lexer.token() == Token.FULLTEXT || lexer.identifierEquals(SPATIAL)) {
      if (replace) {
        lexer.reset(markBp,markChar,Token.CREATE);
      }
    ...
    if (lexer.token() == Token.LOCK) {
      ...
      for (; ; ) {
        ...
        if (lexer.identifierEquals(READ)) {
          ...
          throw new ParserException("syntax error, expect READ or WRITE OR AS, actual " + lexer.token() + ", "+ lexer.info());
        }
        if (lexer.token() == Token.HINT) {
...
parseAlterTableAddColumn(stmt);
 ...
 else if (lexer.token() == Token.INDEX || lexer.token() == Token.FULLTEXT || lexer.identifierEquals(FnvHash.Constants.SPATIAL)) {

...
import com.alibaba.druid.sql.parser.CharTypes.isFirstIdentifierChar;
import com.alibaba.druid.sql.parser.CharTypes.isIdentifierChar;
import com.alibaba.druid.sql.parser.CharTypes.isWhitespace;
import com.alibaba.druid.sql.parser.LayoutCharacters.EOI;
import com.alibaba.druid.sql.parser.SQLParserFeature.KeepComments;
import com.alibaba.druid.sql.parser.SQLParserFeature.OptimizedForParameterized;
import com.alibaba.druid.sql.parser.SQLParserFeature.SkipComments;
 ...
 */
public class Lexer {
  static SymbolTable symbols_l2=new SymbolTable(512);
  int features=0;
  public String text;
  int pos;
  int mark;
  ...
  int bufPos;
  Token token;
  Keywords keywods=Keywords.DEFAULT_KEYWORDS;
  String stringVal;
  long hash_lower;
  long hash;
  int commentCount=0;
  ...
  int lines=0;
  String dbType;
  boolean optimizedForParameterized=false;
 ...
 */
  ...
  public String info(){
    ...
    buf.append("pos ").append(pos).append(", line ").append(line).append(", column ").append(column).append(", token ").append(token);
  ...
  public void nextTokenValue(){
    ...
    if (ch == 'n' || ch == 'N') {
      char c1=0, c2, c3, c4;
      if (pos + 4 < text.length() && ((c1=text.charAt(pos + 1)) == 'u' || c1 == 'U') && ((c2=text.charAt(pos + 2)) == 'l' || c2 == 'L') && ((c3=text.charAt(pos + 3)) == 'l' || c3 == 'L') && (isWhitespace(c4=text.charAt(pos + 4)) || c4 == ',' || c4 == ')')) {
        pos+=4;
        ch=c4;
        token=Token.NULL;
        stringVal="NULL";
  ...
  public final void nextTokenNotOrNull(){
    ...
    if ((ch == 'n' || ch == 'N') && pos + 3 < text.length()) {
      ...
      if ((c1 == 'o' || c1 == 'O') && (c2 == 't' || c2 == 'T') && isWhitespace(c3)) {
        pos+=3;
        ch=c3;
        token=Token.NOT;
        stringVal="NOT";
      ...
      char c4;
      if (pos + 4 < text.length() && (c1 == 'u' || c1 == 'U') && (c2 == 'l' || c2 == 'L') && (c3 == 'l' || c3 == 'L') && isWhitespace(c4=text.charAt(pos + 4))) {
        pos+=4;
        ch=c4;
        token=Token.NULL;
        stringVal="NULL";
...
if (ch == '\\' && charAt(pos + 1) == 'N' && JdbcConstants.MYSQL.equals(dbType)) {
scanChar();
scanChar();
token=Token.NULL;
return;
...
if (ch == '*') {
scanChar();
token=Token.BANG_TILDE_STAR;
}
 else {
token=Token.BANG_TILDE;
}
}
 else {
token=Token.BANG;
...
private static long MULTMIN_RADIX_TEN=Long.MIN_VALUE / 10;
private static long N_MULTMAX_RADIX_TEN=-Long.MAX_VALUE / 10;
private static int[] digits=new int[(int)'9' + 1];
static {
for (int i='0'; i <= '9'; ++i) {
digits[i]=i - '0';
}
...
if (comment.indexOf("select") != -1 || comment.indexOf("delete") != -1 || comment.indexOf("insert") != -1 || comment.indexOf("update") != -1 || comment.indexOf("into") != -1 || comment.indexOf("where") != -1 || comment.indexOf("or") != -1 || comment.indexOf("and") != -1 || comment.indexOf("union") != -1 || comment.indexOf('\'') != -1 || comment.indexOf('=') != -1 || comment.indexOf('>') != -1 || comment.indexOf('<') != -1 || comment.indexOf('&') != -1 || comment.indexOf('|') != -1 || comment.indexOf('^') != -1) {
return false;
}
return true;
}

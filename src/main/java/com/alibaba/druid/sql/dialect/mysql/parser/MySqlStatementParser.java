...
public class MySqlStatementParser extends SQLStatementParser {
  ...
  private static String FULLTEXT="FULLTEXT";
  ...
  public SQLStatement parseCreate(){
    ...
    if (lexer.token() == Token.UNIQUE || lexer.token() == Token.INDEX || lexer.identifierEquals(FULLTEXT) || lexer.identifierEquals(SPATIAL)) {
      if (replace) {
        lexer.reset(markBp,markChar,Token.CREATE);
      }
  ...
  public boolean parseStatementListDialect(  List<SQLStatement> statementList){
    ...
    if (lexer.token() == Token.HINT) {
      ...
      if (hints.size() == 1 && statementList.size() == 0 && (token == Token.SELECT || token == Token.DELETE)) {
  ...
  public SQLBlockStatement parseBlock(){
    ...
    if (lexer.token() != Token.END && statementList.size() > 0 && (statementList.get(statementList.size() - 1) instanceof SQLCommitStatement || statementList.get(statementList.size() - 1) instanceof SQLRollbackStatement)) {
  ...
  @Override public SQLRollbackStatement parseRollback(){
  ...
  SQLStatement parseAlterTable(  boolean ignore){
    lexer.nextToken();
 ...
 else         if (lexer.token() == Token.INDEX || lexer.identifierEquals(FnvHash.Constants.FULLTEXT) || lexer.identifierEquals(FnvHash.Constants.SPATIAL)) {
 ...
 * 定义异常处理程序
 * @return
 * @author zhujun [455910092@qq.com]2016-04-16
 */

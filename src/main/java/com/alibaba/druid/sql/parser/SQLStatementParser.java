...
public class SQLStatementParser extends SQLParser {
  ...
  boolean parseCompleteValues=true;
  int parseValuesSize=3;
  SQLSelectListCache selectListCache=null;
  ...
  public void parseStatementList(  List<SQLStatement> statementList,  int max,  SQLObject parent){
    if ("select @@session.tx_read_only".equals(lexer.text) && lexer.token == Token.SELECT) {
...
statementList.add(stmt);
if (parent instanceof SQLBlockStatement && JdbcConstants.MYSQL.equals(dbType)) {
return;
}
continue;
}
...
privilege="SUPER";
 ...
 else if (lexer.identifierEquals("CONTROL")) {
lexer.nextToken();
privilege="CONTROL";
}
 else if (lexer.identifierEquals("IMPERSONATE")) {
lexer.nextToken();
...
if (lexer.identifierEquals("CONSTRAINTS")) {
lexer.nextToken();
}
continue;
}
...
return stmt;
 ...
 else if (token == Token.INDEX || token == Token.UNIQUE || lexer.identifierEquals("NONCLUSTERED")) {
...
if (lexer.token == Token.COMMA || lexer.token == Token.OR) {
lexer.nextToken();
continue;
}
break;
...
/** 
 * @param tryBest  - 为true去解析并忽略之后的错误强制建议除非明确知道可以忽略才传tryBest=true, 不然会忽略语法错误，且截断sql,导致update和delete无where条件下执行！！！
 */
...
if (lexer.identifierEquals("FORMAT") || lexer.identifierEquals("EXTENDED") || lexer.identifierEquals("PARTITIONS")) {
